...
class ArmRigBuilder:
    ...
    ctrl_of_choice: dropdown("Shape", "Control Setup Info", "Arrow", "FourDArrow", "TwoDArrow", "Plus", "Circle",
                             "Square") = "Circle"
    ...
    def create_ik_controllers(self):
        ...
        ik_handle = cmds.ikHandle(n=self.ik_handle_name, sj=selected_joints[0], ee=selected_joints[1], s=False,
                                  snc=False)
...
class MetadataFragment:
    ...
    def __repr__(self):
        return f"This fragment relates to the field {self.bind[1]} of {self.bind[0]}"
    ...
    def extract_reflection(target: object) -> dict[str, list]:
        ...
        fragment_groups = {}
        ungrouped_fragments = []
        defaults_values = type(
            target).__dict__  # Get all the variable names and default values of the target object reference

        for field_name, field_metadata in target.__annotations__.items():  # For each metadata blob acquired from the target
            if isinstance(field_metadata, _AnnotatedAlias) and isinstance(field_metadata.__metadata__[0],
                                                                          Element):  # Check if that blob is about this system
                fragment = MetadataFragment(target, field_name, defaults_values[field_name],
                                            field_metadata)  # Build the reflection fragment using the metadata blob
                (fragment_groups.setdefault(fragment.group, []) if fragment.group else ungrouped_fragments).append(
                    fragment)  # adds it to the groups or ungroup if it has a group
...
class Window:
    ...
    def assemble_window(self) -> str:
        ...
        bucket_tab_element = self.assemble_abstract_tab(self.bucket_tool_ref,
                                                        self.arm_control_box_widget)  # Build the first Tab "Bucket tool"

        cmds.tabLayout(tabs_element, edit=True,
                       tabLabel=(bucket_tab_element, 'Bucket'))  # Attach the bucket tool tab to the tabs element
    ...
    @staticmethod
    def create_int_slider_widget(*args) -> str:
        return Window.create_abstract_slider_widget(cmds.intSlider, cmds.intField, *args)[0]

    @staticmethod
    def create_float_slider_widget(*args) -> str:
        root_element, text_field_element, slider_element = Window.create_abstract_slider_widget(cmds.floatSlider,
                                                                                                cmds.floatField, *args)
        cmds.floatField(text_field_element, edit=True, tze=False, value=args[1])
        return root_element

    @staticmethod
    def create_abstract_slider_widget(cmds_slider_func, cmds_field_func, bind, default, label, min_range, max_range,
                                      *_):
        ...
        def on_update(value, *_):
            cmds_field_func(text_field_element, edit=True,
                            value=round(float(value), 2))  # update the text field element with the new info
            cmds_slider_func(slider_element, edit=True,
                             value=round(float(value), 2))  # update the slider element with the new info
            setattr(*bind, value)  # overwrite the bound variable's value with the new value

        root_element = cmds.rowLayout(numberOfColumns=3, adjustableColumn3=3, columnWidth3=(75, 35, 30),
    ...
    def create_dropdown_widget(bind, default, label, choices, *_):
        ...
        root_element = cmds.rowLayout(numberOfColumns=2, adjustableColumn2=2, columnWidth2=(75, 70),
    ...
    def create_toggle_widget(bind, default, label, *_) -> str:
        ...
        root_element = cmds.rowLayout(numberOfColumns=3, adjustableColumn3=3, columnWidth3=(75, 66, 53),
                                      columnAlign3=["right", "left", "right"], columnAttach3=["both", "both", "right"])
        cmds.text(label=f"{label}:", align="right", font="boldLabelFont")
        cmds.iconTextRadioCollection()
        cmds.iconTextRadioButton(st='textOnly', l='TRUE', hlc=Window.UI_GREEN, bgc=Window.UI_BGC,
                                 font="smallFixedWidthFont", fla=False, select=default,
                                 cc=lambda value, *_: setattr(*bind, value))
        cmds.iconTextRadioButton(st='textOnly', l='FALSE', hlc=Window.UI_RED, bgc=Window.UI_BGC,
                                 font="smallFixedWidthFont", fla=False, select=not default)
        cmds.setParent("..")
        return root_element
    ...
    def assemble_abstract_tab(target_reference, control_box) -> tuple[str, str, str]:
        ...
        # draw icon box
        image_box_element = cmds.iconTextStaticLabel(st='iconOnly', i='PxrPtexture.svg', l='spotlight',
                                                     bgc=Window.UI_BGC)

        #  draw prompt box
        prompt_box_element = cmds.text(label=""" - test\n - test\n - test""", ww=True, al="left", font="fixedWidthFont",
                                       bgc=Window.UI_BGC)

        # draw button box, and pass the key methods it needs to be able to control the entire tab
        contrl_box_element = control_box(
            lambda text: cmds.text(prompt_box_element, edit=True, label=text),
            # This method controls the prompt box's texts
            lambda icon: cmds.iconTextStaticLabel(image_box_element, edit=True, i=icon),
            # This method controls the image box's images
            lambda state: cmds.scrollLayout(tuning_box_element, edit=True,
                                            enable=state))  # This method controls the tuning box's enabled status

        # attach elements to form
        cmds.formLayout(main_element, edit=True,
                        attachPosition=[
                            (prompt_box_element, 'top', 1, 6), (prompt_box_element, 'bottom', 2, 8),
                            (prompt_box_element, 'left', 2, 0), (prompt_box_element, 'right', 2, 8),
                            (tuning_box_element, 'top', 1, 0), (tuning_box_element, 'bottom', 1, 6),
                            (tuning_box_element, 'left', 1, 0), (tuning_box_element, 'right', 0, 5),
                            (contrl_box_element, 'top', 2, 3), (contrl_box_element, 'bottom', 2, 6),
                            (contrl_box_element, 'left', 2, 5), (contrl_box_element, 'right', 2, 8),
                            (image_box_element, 'top', 2, 0), (image_box_element, 'bottom', 1, 3),
                            (image_box_element, 'left', 2, 5), (image_box_element, 'right', 2, 8)])
...
class ArrowCtrlBuilder(ControllerBuilder):
    ...
    def build(self):
        cmds.curve(n=self.ctrlName, d=1,
                   p=[(0, 0, -1), (2, 0, -1), (2, 0, -2), (4, 0, 0), (2, 0, 2), (2, 0, 1), (0, 0, 1), (0, 0, -1)])
